...
Declared phases are part of the plan: a profile may list them (`taskPlanning.phases: [{id: E0, title: Recon}]`), a task joins one at creation (`create_task phase: 'E0'`) or later (`move_phase`), and the board and the task checklist order themselves by them — falling back to the DAG levels when nothing is declared.

**A phase can be closed, and a closed phase takes no new work.** Closing is the captain's own bookkeeping step after he accepted the tasks of that phase: one `close_phase` operation of the same replan batch (`{ action: 'close_phase', phase_id: 'E0' }`), allowed only when every task in the phase is terminal — otherwise it is refused naming the phase and the tasks still open (`phase "E1" still has 1 open task(s): t2 (pending) — accept and settle them first`). A `failed` task counts as settled: it is evidence of a real attempt and Delivery, not the phase, is what judges it. Once closed, the phase refuses `create_task phase: 'E0'` and a replan `add_task`/`move_phase` into it, and the refusal points at the only way forward — declare a new phase (`move_phase`/`add_task` with a new `phase_id` and a `title`). The snapshot publishes `closed` (and `closedAt`) per phase: the board draws that column's underline in the delivered tone with a `Closed` marker and never offers the phase as a target in the running-plan editor, so a reader sees why the column stopped accepting work. A `team.json` written before this field exists keeps loading, with every phase open.
...
**One bar, three colours.** The percentage stays one number, but the bar behind it is split into the three stretches of the plan's life: **the original plan** (blue), **what the captain added while it ran** (amber) and **what arrived after the plan had settled** (green) — a user's later request reaches the team as captain-created work, so this is how it is told apart. Each zone is as wide as its stretch is big and fills with the share that stretch delivered, and the legend under the bar names all three with their counts (`Original plan 14/16 · Added while running 3/5 · Added after the plan 1/3`). The attribution is recorded on the task when it is created (`origin`: `plan`, `added` or `followup`) and never moves afterwards, so repairing or reopening the plan cannot reshuffle history; a task with no recorded origin counts as plan work, which keeps a state file written before this field working. The replan tool is the captain's, so no tool call distinguishes "the user asked" — the moment of creation does: work created while the plan still owes something is `added`, work created after every original task settled is `followup`.
