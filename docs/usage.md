...
| Web server route registration | the activity-panel data route `/plugins/dsh-agent-teams/state` plus static whale artwork (dual-key compatibility for `webServer`/`httpServer`, see below) |
...
- **Whale artwork**: captain/member avatars are DeepSeek whale occupational illustrations (`assets/agent-teams/`, 8 roles + 6 actions) matched by role keyword; the state action glyph follows the member state with an animation (working float / idle breathe / unknown thinking), an unread message adds a halo around the avatar; `prefers-reduced-motion` is respected.
