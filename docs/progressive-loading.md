...
`pnpm verify:capabilities` uses the real scoped registry, prompt assembly and WorkerThreadCodeRuntime, covering the fixed rules, direct business calls to the 16 tools, duplicate creation retaining the original team, a user restriction, cancel/failure, trusted member identity, pause, cold resume, HMR, PTC/both SDKs and actual result truncation.
