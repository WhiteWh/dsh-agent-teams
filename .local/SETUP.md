...
**Теги релизов (правило владельца, 2026-09-20):** каждый релиз получает
**аннотированный** тег `v<package.version>`, и в сообщении первой строкой идёт
пометка ветки — `AgentTeams <version> — branch agent-teams-hardening (fork
release)`. Наши теги не апстримовые (у NanmiCoder теги кончаются на `v0.1.20`),
пометка нужна, чтобы линии не путались в клоне с двумя remote. Тег ставится
**после** сборки и установки артефакта, указывает на коммит, из которого собран
этот артефакт, и содержит его имя, размер и SHA-256 плюс результат локальной
проверки. Докоммит после релиза тег не двигает. Проставлено: `v0.1.21` →
`12f245b`, `v0.1.22` → `9f81203`; оба запушены в `fork`.
