...
| **One-line member node** | The roster row is a single compact line — 24 px portrait, role symbol (words in the tooltip), name, model badge, action symbol with its state label and the member's task chips — ending in the full-height animated work plaque. The retired role words, status sentence and their locale keys went with the old two- and three-line layout. | `src/client/ActivityPanel.tsx`, `src/client/ActivityPanel.module.css` |
