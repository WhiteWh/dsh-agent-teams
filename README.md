...
[v0.2.2](./release-notes/v0.2.2.md) is a panel-and-contract release: a member row is one compact line (portrait, role symbol, name, model badge, action symbol with its state label, task chips) ending in the work plaque; the phase board lays sequential work inside a phase out as a line and stretches the column to its longest chain, a cancelled node is a pale scarlet hatched card, and the Tree and Queues views are gone with their model projections — the board is the panel's single graph view and the progress block is one bar again. It also fixes a regression shipped in [v0.2.1](./release-notes/v0.2.1.md): a plan whose declared phases covered every task rendered an empty board.

[v0.2.1](./release-notes/v0.2.1.md) is a panel-visual release: in the members tree a member's work is the node's own height — a plaque three dots wide running down the row with a top-to-bottom wave while that member works — instead of the compact six-dot mark inside its text. The client bundle is the only thing that changed; no state, tool, schema or gate moved.
...
| **One graph view: the phase board** | Every phase is a column and every dependency is a real SVG curve. Work that runs in sequence inside a phase reads as one line, parallel work takes its own row, and the column stretches to the width of its longest chain — which is why the separate Tree and Queues views (and their model projections) are gone. A cancelled node is painted in a pale scarlet hatch as settled history rather than in the alarm colour. | `src/client/activity-model.ts`, `src/client/ActivityPanel.tsx` |
| **Large member node, foldable to a tray** | A member is large by default: the portrait spans both lines with the live action in its corner, the head line carries the role in words, the model badge and the state word, the line beneath is the sentence behind that state, and the task chips sit behind their label. A chevron at the node's right edge folds that one member into a compact tray line (role symbol, name, model, state, chips) without touching its neighbours, and the tray keeps the chevron to unfold again. | `src/client/ActivityPanel.tsx`, `src/client/ActivityPanel.module.css` |
...
| **Plan progress and a task checklist** | One percentage per team, computed on the server and shared by the panel, the conversation card and the `agent_teams_status` text (`Progress: 62% (8/13; running 2, blocked 1, failed 0, waived 1)`). Cancelled, superseded and already-repaired work leaves the denominator; `byKind` (implementation 3, repair 2, rest 1) and `equal` are both reported, `taskPlanning.weights` picks the default and the panel switches it. **The bar is one line in three colours** — the original plan, what the captain added while it ran, and what arrived after the plan had settled — each zone as wide as its stretch is big and filled with its own delivered share. The attribution is stamped on a task when it is created (`origin`: `plan`/`added`/`followup`) and never moves, so repairing the plan cannot reshuffle it. A collapsible checklist lists every task in phase-then-depth order with its status glyph, waivers and `→ tN` replacement link, and clicking a row pins that node in the board. | `src/progress.ts`, `src/state.ts`, `src/snapshot.ts`, `src/tools.ts`, `src/client/ActivityPanel.tsx`, `src/client/AgentTeamsCard.tsx` |
| **Replanning a live team** | `agent_teams_replan` repairs a running plan in one atomic batch — `add_task`, `update_task` (with `retry` and `invalidate`), `supersede_task`, `cancel_task`, `accept_paths`, `amend_task`, `move_phase`, `close_phase` — under one reason, validated as a whole so a single bad step writes nothing. `invalidate: true` revokes the attempt a member holds, drains it, leaves `task tN replanned: …` in its mailbox, and a revoked capability is refused afterwards. Every batch bumps `plan.revision` and appends an `agent-teams/plan-revised` diff; declared phases (`taskPlanning.phases`, `create_task phase`, `move_phase`) drive the board columns and the checklist; the panel ships a running-mode editor that posts the same batch to `POST /plan` with `action: 'replan'`. | `src/replan.ts`, `src/tools.ts`, `src/state.ts`, `src/index.ts`, `src/client/ActivityPanel.tsx` |
| **A phase closes when it is done** | After accepting a phase's tasks the captain closes it (`close_phase`), which is allowed only when every task in the phase is terminal; a closed phase refuses `create_task` and a replan `add_task`/`move_phase` into it and names the way forward — declare a new phase. The snapshot publishes `closed`/`closedAt`, the board marks the column as settled, and the running-plan editor never offers it as a target, while a `team.json` written before the field keeps loading with open phases. | `src/replan.ts`, `src/tools.ts`, `src/types.ts`, `src/state.ts`, `src/snapshot.ts`, `src/client/ActivityPanel.tsx` |
| **Several teams at once** | The panel switches teams: one tab per live team (name, `done/total`, activity dot) and one team rendered at a time — its DAG, members, progress and slices — with the choice remembered and a halted team keeping its tab. The scheduler sweeps every live team of every workspace (`agent_teams_status` on a multi-team captain kicks the sweep) and two fuses bound concurrent work: `maxWorkersPerTeam` (defaults to the roster cap) and `maxConcurrentWorkersGlobal` (default 8), enforced on the dispatch primitive so no kick path can exceed them. | `src/client/activity-model.ts`, `src/client/ActivityPanel.tsx`, `src/scheduler.ts`, `src/index.ts` |
| **Configurable limits and slot reporting** | All four fuses are profile config with defaults — `maxTeamsPerWorkspace` 4, `maxTeamsPerSession` 8, `maxWorkersPerTeam` (roster cap), `maxConcurrentWorkersGlobal` 8 — resolved in one place so the create guard, the scheduler and the report agree. `agent_teams_status` names who holds a slot and how much waits (`Slots: 1/4 working (worker t2); 3 queued`), lists the limits in force, and carries the same summary as data (`slots.*`, `limits.*`). | `src/tools.ts`, `src/index.ts`, `src/scheduler.ts` |

Every plan work package is complete on this branch (WP1–WP11); the release history is
the list above, and this README documents the current release.
...
| **Live activity panel** | The Web UI combines segmented progress, a collapsible roster, and the interactive phase board; running tasks show the member's model, and completed archives retain their full member and task history. The board lays each phase out as a column — sequential work as one line, parallel work on its own row, the column stretching to its longest chain — and a clicked node opens its detail underneath. |
...
**Recommended pair: DeepSeek Harness `0.1.5-rc.1` + AgentTeams `0.3.0`. Harness remains a prerelease.**

| Use case | DeepSeek Harness | AgentTeams plugin |
| --- | --- | --- |
| **Recommended installation** | **`0.1.5-rc.1`** | **`0.3.0`** |
| Retaining an older RC | `0.1.2-rc.1` | `0.3.0` |
| Developer Alpha testing | `0.1.2-alpha.5` | `0.3.0` |
| Retaining an older Alpha | `0.1.2-alpha.2` | `0.3.0` |
...
```sh
dsh plugin --profile web add --save-exact @nanmicoder/dsh-agent-teams@0.3.0
```

**After installation, stop and restart Harness for that profile, then refresh the browser.**

The default npm `latest` tag points to `0.3.0`, so `dsh plugin --profile web add @nanmicoder/dsh-agent-teams` installs this version on a fresh profile. Use the exact-version command above to pin it. The recommended Harness version is `0.1.5-rc.1`; installing the plugin does not upgrade the host. See the [source installation guide](./docs/maintenance-workflow.md) and [release verification](./docs/releases/v0.3.0/README.md).
