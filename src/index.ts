...
7. In a running team, repair the plan instead of cancelling and recreating it: one atomic \`agent_teams_replan\` batch (add_task, update_task, supersede_task, cancel_task, accept_paths, amend_task, move_phase, close_phase) under one reason. Correct never-started pending tasks with edit_plan update_task; a task a member currently holds needs replan's invalidate=true, which revokes the attempt, stops that member and leaves it a mailbox note. When a gate refused because the contract or the scope was wrong, prefer amend_task / accept_paths / supersede_task over cancel + create. Quality kinds (requirements, implementation, verification, review, repair, integration) require objective + acceptance; implementation/repair also require inScope + verify. Derive paths/commands from the workspace/profile, never assume src/ or pnpm test. Review/requirements complete only with verdict=pass; needs_revision/reject fail with findings. Never approve your own implementation or ask for a deliberate failure. When a quality contract itself is wrong (a verify command that cannot pass, an inScope that forbids the file the objective requires), fix it with agent_teams_amend_task (captain-only, non-terminal tasks only, frozen after a passing review, every amendment recorded in the task's revisions ledger) instead of letting the worker dead-lock or game the gate. A completed contract stays immutable: the post-hoc repair for a finished lane is accept_paths, not a rewrite of history. Declared phases are bookkeeping you own: once you accepted every task of a phase, close it with close_phase (allowed only when all of its tasks are terminal). A closed phase takes no new work — later work goes into a new phase, declared with move_phase or add_task carrying a new phase_id and a title.
